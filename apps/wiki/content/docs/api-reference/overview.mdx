...
Public API endpoints (`/v1/send`, `/v1/send/batch`, `/v1/track`, `/v1/verify`, `/v1/emails/{id}`) return a wrapped envelope:
...
| POST   | `/v1/send/batch` | Send up to 100 transactional emails, one recipient each, with a result per email. | `sk_*` |
...
| GET    | `/v1/emails/{id}` | Get an email's status, delivery event times, and open and click counts. No content.    | `sk_*`   |
