...
| `EMAIL_SEND_ATTEMPTS`         | No       | Attempts per email. Retried: SES throttling and server errors, failures to reach SES, and failures before the message is handed to SES. Attempts also cover recording a message SES accepted when that first failed, so with `1` such an email stays `SENDING`. Emails keep the value they were queued with. From `1` to `25`. | `3`     |
| `EMAIL_SEND_BACKOFF_MS`       | No       | Delay before an email's first retry, in milliseconds; it doubles for each retry after it. Emails keep the value they were queued with. At most `3600000`. | `2000`  |
