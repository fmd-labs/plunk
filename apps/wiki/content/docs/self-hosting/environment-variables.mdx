...
| `EMAIL_STALL_SWEEP_MAX_AGE_HOURS` | No       | Hours after its creation past which the sweep fails an email rather than send it late, without an `email.failed` event: emails an earlier version left behind are not sent days later. From `1` to `8760`. | `24`    |
