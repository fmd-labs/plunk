...
| Variable                    | Required | Description                                                                                                         | Default |
| --------------------------- | -------- | ------------------------------------------------------------------------------------------------------------------- | ------- |
| `EMAIL_BODY_RETENTION_DAYS` | No       | Days a sent email keeps its rendered HTML body before the daily cleanup clears it; the email itself is kept, and an email still waiting to be sent keeps its body. `0` keeps every body; the maximum is `36500`. | `90`    |
...
| `EMAIL_SEND_ATTEMPTS`         | No       | Attempts per email. Retried: SES throttling and server errors, failures to reach SES, and failures before the message is handed to SES. Attempts also cover recording a message SES accepted when that first failed, so with `1` such an email stays `SENDING`. Emails keep the value they were queued with. From `1` to `10`. | `3`     |
