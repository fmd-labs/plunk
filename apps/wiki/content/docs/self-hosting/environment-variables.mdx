...
| `SOURCE_CODE_URL` | No       | Where the source code of this deployment is published. When set, the dashboard and the unsubscribe, subscribe and manage pages link to it: a modified version run as a network service has to offer its source to its users (AGPL-3.0 section 13). Must be an `http(s)` URL. | —       |
