...
| `EMAIL_SEND_ATTEMPTS`         | No       | Attempts per email. Retried: SES throttling and server errors, failures to reach SES, and failures before the message is handed to SES. Emails keep the value they were queued with. | `3`     |
| `EMAIL_SEND_BACKOFF_MS`       | No       | Delay before an email's first retry, in milliseconds; it doubles for each retry after it. Emails keep the value they were queued with.                     | `2000`  |
