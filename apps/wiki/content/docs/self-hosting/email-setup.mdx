...
`ses:GetSendQuota` lets the email worker read your account's sending rate when it starts, unless `EMAIL_RATE_LIMIT_PER_SECOND` sets the rate; without either, the worker falls back to 14 emails per second. `ses:DeleteIdentity` removes a domain's SES identity when the domain is deleted from Plunk and no other project uses it.
...
SNS delivers an event again only when Plunk answers with a server error, by default three more times, 20 seconds apart. Plunk does so when it cannot read its database, and for a delivery, open, click, bounce or complaint of a message it has not recorded yet, up to an hour after SES accepted the message, unless the message went out with a configuration set other than Plunk's two: on a topic several deployments share, another's events are not delivered again. To keep the events of a longer outage, set a delivery policy on the subscription that retries for longer (**Delivery policy (HTTP/S)** in the subscription's settings), such as AWS's example of 50 retries over about 40 minutes:
