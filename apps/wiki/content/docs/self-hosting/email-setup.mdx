...
`ses:GetSendQuota` lets the email worker read your account's sending rate when it starts; without it, the worker falls back to 14 emails per second. `ses:DeleteIdentity` removes a domain's SES identity when the domain is deleted from Plunk and no other project uses it.
